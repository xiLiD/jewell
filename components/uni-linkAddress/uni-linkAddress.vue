						...
						<view class="operation-container">
							<view class="operation-content" v-for="(base_items, base_index) of linkAddress_area">
								<scroll-view scroll-y="true" class="province" show-scrollbar="false">
									...
									>
										{{ items.Name }}
										<view class="pic" v-show="index === base_items.current"><image src="../../static/xuan-linkAddress/yes.png"></image></view>
									</view>
								</scroll-view>
							</view>
						</view>
	...
	methods: {
		...
		show: function() {
			...
			(this.submission = {
				...
				//请求省数据
				var _this = this;
				_this.$request.furnitureOrder
					.GetProvince({})
					.then(data => {
						if (data.status == 1) {
							_this.linkAddress_area[0].info = data.datas;
						}
					});
		...
		province_txt_click(target) {
			...
			//请求市数据
				...
				_this.$request.furnitureOrder
					.GetCity({
						provinceId:_this.submission.province
					})
					.then(data => {
						if (data.status == 1) {
							_this.linkAddress_area[0].info = data.datas;
